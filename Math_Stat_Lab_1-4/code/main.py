...
def task1():
    N = [10, 50, 100]#sizes of series of data
    for distribution in distType:#for each distribution
        ...
        if not os.path.isdir("MathStat_lab_1"):
          os.makedirs("MathStat_lab_1")
        plt.savefig("MathStat_lab_1/" + distribution + ".png")#save result
    plt.show()
    ...
    for distribution in distType:
        ...
        for size in N:
            ...
            result.append([" E(z) + sqrt(D(z)) " + str(size),
                           ...
                           "ZQ: " + str(np.around(calculateMean(ZQ) + np.std(ZQ), decimals = 6)),
                           "ZTR :" + str(np.around(calculateMean(ZTR) + np.std(ZTR), decimals = 6))])
        if not os.path.isdir("MathStat_lab_2"):
            os.makedirs("MathStat_lab_2")
        fileName = distribution + "_data"
        completeName = os.path.join("MathStat_lab_2/", fileName + ".txt")
...
def task3():
    ...
    for distribution in distType:#for each distribution
        ...
        if not os.path.isdir("MathStat_lab_3"):
          os.makedirs("MathStat_lab_3")
        plt.savefig("MathStat_lab_3/" + distribution + ".png")#save result
        plt.figure()
    #outliers
    result = []
    for distribution in distType:
    ...
    if not os.path.isdir("MathStat_lab_3"):
      os.makedirs("MathStat_lab_3")
    completeName = os.path.join("MathStat_lab_3/", "outliers.txt")
    file = open(completeName, "w")
    for element in result:
      file.write(str(element) + "\n")
    file.close()
...
def task41():
    ...
    for distribution in distType:
        ...
        for elem in array:
            ...
            if not os.path.isdir("MathStat_lab_4_1"):
                os.makedirs("MathStat_lab_4_1")
            plt.savefig("MathStat_lab_4_1/" + distribution + ".png")#save result
            index += 1
    ...
    for distribution in distType:
        ...
        for elem in array:
           ...
           for k in koef:
               ...
               axes[i].legend()
               i = i + 1
               if not os.path.isdir("MathStat_lab_4_2"):
                   os.makedirs("MathStat_lab_4_2")
               plt.savefig("MathStat_lab_4_2/" + distribution + "KDE" + str(N[index - 1]) + ".png")#save result
           index += 1
